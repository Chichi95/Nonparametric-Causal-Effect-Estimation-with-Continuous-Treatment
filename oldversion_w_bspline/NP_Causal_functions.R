...
gram_schmidt <- function(polylist,range){
  len <- max(length(polylist),3)
  inner_prod <- function(poly1, poly2){
    integral(poly1*poly2, limits = range) 
  }
  #first basis
  othno_poly <- list()
  othno_poly[[1]] <- polylist[[1]]/sqrt(inner_prod(polylist[[1]],polylist[[1]]))
  #second basis
  unnorm_ver <- polylist[[2]] - inner_prod(polylist[[2]],othno_poly[[1]]) * othno_poly[[1]]
  othno_poly[[2]] <- unnorm_ver/sqrt(inner_prod(unnorm_ver,unnorm_ver))
  for (i in 3:len){
    unnorm_ver <- othno_poly[[i-1]]*polynomial(c(0,1)) - inner_prod(othno_poly[[i-2]],othno_poly[[i-1]]*polynomial(c(0,1)))*othno_poly[[i-2]] 
    - inner_prod(othno_poly[[i-1]],othno_poly[[i-1]]*polynomial(c(0,1)))*othno_poly[[i-1]]
    othno_poly[[i]] <- unnorm_ver/sqrt(inner_prod(unnorm_ver,unnorm_ver))
  }
  othno_poly
}
Powerseries_construct_T <- function(X ,df, range){
  ...
  othno_poly <- gram_schmidt(polylist, range[[1]])
  for (i in 1:length(X)){
    for (j in 1:df){
